...
import { ScanFace, Shield, Check, XCircle, Camera } from "lucide-react";
...
const FaceIdAuth: React.FC<FaceIdAuthProps> = ({ 
  onAuthenticate, 
  onCancel,
  enrollmentMode = false 
}) => {
  // Authentication states
  const [scanningState, setScanningState] = useState<"idle" | "requesting" | "scanning" | "analyzing" | "success" | "error" | "denied">("idle");
  ...
  const requestCameraPermission = async () => {
    ...
    try {
      ...
      toast.error("Camera access denied. Face ID requires camera permission.", {
        position: "top-center",
      });
    }
  };
  ...
  const startFaceDetection = () => {
    ...
    const storedMetrics = !enrollmentMode ? localStorage.getItem('faceIdMetrics') : null;
  ...
  const processFaceAuthentication = () => {
    ...
    if (enrollmentMode) {
      if (enrollmentStep < 3) {
        ...
        // Complete enrollment
        localStorage.setItem('faceIdMetrics', faceMetrics);
    ...
    const storedMetrics = localStorage.getItem('faceIdMetrics');
    ...
    // Simulate face matching (in a real app, this would be a proper comparison)
    // Here we're just checking that both metrics exist
    const isMatch = storedMetrics && faceMetrics;
      ...
      <Card className="max-w-md w-full mx-4 bg-black/80 border-gray-800 text-white shadow-[0_0_15px_rgba(0,123,255,0.5)]">
        ...
        <CardContent className="flex flex-col items-center py-8 px-4">
          <AnimatePresence mode="wait">
              ...
              >
                <div className="w-64 h-64 overflow-hidden relative border-2 border-blue-500 rounded-lg flex items-center justify-center bg-black">
                  ...
                  <div className="absolute inset-0 z-10">
                    ...
                    {/* Facial landmark indicators (simulated) */}
                    {faceDetected && (
                      <div className="absolute inset-0 pointer-events-none">
                        {/* Face detection box will be drawn on the canvas */}
                      </div>
                    )}
                  </div>
            ...
            {(scanningState === "error" || scanningState === "denied") && (
              <motion.div 
                key="error"
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
